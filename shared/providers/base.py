...
class BaseVideoProvider(ABC):
    ...
    def calculate_cost(self, request: VideoGenerationRequest) -> float:
        ...
        if request.image_url and capabilities.get("supports_image_input", False):
            base_cost *= capabilities.get("image_cost_multiplier", 1.5)

        base_cost *= self._get_resolution_multiplier(request.resolution_width, request.resolution_height)
